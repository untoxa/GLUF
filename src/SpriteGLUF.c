	...
	while (TRUE) {
			...
			if (KEY_PRESSED(J_UP)) {
				...
				if (player_x < (LEVEL_WIDTH - 1)) {
					if (check_collision(level_buffer[player_y][player_x + 1]) == 0) {
						...
						ExecuteSFX(BANK(sfx1jump), sfx1jump, SFX_MUTE_MASK(sfx1jump), SFX_PRIORITY_NORMAL);
		...
		if (lifting == TILE_LIFT_UP) {
			...
			if (player_y < LEVEL_HEIGHT) {
				if (check_collision(tile_below) == 0) {
					...
					player_y++;
					SetSpriteAnim(THIS, anim_fall, ANIMATION_SPEED_FALL);
					ExecuteSFX(BANK(sfx3fall), sfx3fall, SFX_MUTE_MASK(sfx3fall), SFX_PRIORITY_NORMAL);
		...
		if (tile_below == TILE_BATT_DISCHARGED) {
			if (charge) {
				...
				ExecuteSFX(BANK(sfx5paint), sfx5paint, SFX_MUTE_MASK(sfx5paint), SFX_PRIORITY_HIGH);
			}
			...
			if (charge_cooldown) {
				...
				ExecuteSFX(BANK(sfx6noname), sfx6noname, SFX_MUTE_MASK(sfx6noname), SFX_PRIORITY_NORMAL);
			}
		}
		// exit
